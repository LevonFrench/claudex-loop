...
- xloop: S6 user-correction capture and the closing rating. `QUESTIONS.md` gains correction records (`Correction [<phase>/<round>]:` / `Ruling: user_right | agent_right | unresolved` / `Evidence:`), appended by the new `loop-step.ps1 -Transition record-correction`, which refuses a ruling without evidence. Closeout promotes every `user_right` correction and every answer that overrode `Recommended:` into the loop's lesson note as `[user-ruling]` with recommendation and ruling side by side; `loop-status.ps1 -Corrections` derives that list clerically and drops malformed records. After `done` the driver asks one closing question (rate 1-5, default skip, a rating of 3 or lower carries one `Feedback:` line); `loop-step.ps1 -Transition record-rating` writes `.loop/RATING.md`, promoted as `[rating]`, and a skipped rating writes nothing. The closeout packet now names the question batch.
- xloop: S8 supersession for lessons and settled decisions. Lesson notes and settled-decision rows carry `supersedes:` and `superseded-by:`; PLAN §D rows may declare `Supersedes: <loop>/<Dn>`; closeout writes both sides of a supersession in one upsert. Recon's bounded lessons grep excludes every note whose `superseded-by:` is set, implemented as `Get-LoopRecentLessons` in `loop-common.ps1` and `loop-status.ps1 -Lessons`, with the equivalent one-liner stated in `references/recon.md`.
