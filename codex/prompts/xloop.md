...
Authorization is settled by this invocation. The user starting or resuming the loop authorizes every wrapper summon, every packet sent to Claude, and Claude's writes and commits inside the project during build. Never ask the user to authorize a summon, never add an authorization question to `QUESTIONS.md`, and never relay a summoned agent's request for approval: summoned agents have no human and their wrappers reject any file they were not assigned.
