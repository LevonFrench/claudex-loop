...
Windows read-intent calls explicitly use `approval_policy="on-request"` with `approvals_reviewer="auto_review"`, so a captured reviewer can route eligible command approvals through automatic review. Fresh and resumed calls retain the workspace-write sandbox and packet guards; a denied command remains a blocker. See the [OpenAI configuration reference](https://learn.chatgpt.com/docs/config-file/config-reference).
...
| 20260905T024033 | codex | none | FAIL (6 of 7 mechanical steps passed; inspection invalid) | `tests/out/takeover-20260905T024000/live-loop-codex-none-20260905T024033.md` (local, untracked), source fix `0bbcccf`. The kill at review round 3, replay, and recap-free resume passed. Claude completed five review rounds and build round 1, with both proof rungs reported passing. Codex inspection could not read its assigned evidence because shell commands were rejected by policy; its format retry failed for the same reason. The driver then overwrote the failed inspection with a bare approval and exited 0 at `closeout_step: brief`, without summoning closeout (the scaffolded closeout report is still empty). This is not valid inspection or acceptance. The no-wiki scenario and remaining matrix require a new valid run. |

Manual observations for the third run: round 1's durable reviewer file contains five finding IDs, but its response and review ledger accept six, following two wrapper invocations. The metadata also records a guarded ledger mutation. The overwritten inspection artifact and the mismatched review record must not be treated as trustworthy approval evidence. The original transcripts, metadata, malformed artifact, and observation note remain beside the local summary; the failed fixture is preserved unchanged.

The follow-up Windows approval route passed live fresh and resumed read-intent probes against a minimal fixture containing only public repository files, with no guarded mutations. These are access probes, not acceptance passes. The harness now tells drivers to await running wrappers, preserve inspector outputs, and complete closeout, and its final gate rejects approvals that conflict with failed or revised wrapper metadata. The saved third-run approval is rejected by this gate.
