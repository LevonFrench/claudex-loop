...
- Optional visible consoles mirror normalized events. Closing a viewer does not stop its session.
...
- Messages and output are bounded. Peer output is untrusted data and never transfers tool authority.
...
3. Offline tests prove strict label validation, bounded messages, complete MCP discovery, per-session serialization, and cross-session concurrency.
