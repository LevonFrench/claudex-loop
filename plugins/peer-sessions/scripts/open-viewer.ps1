...
function Invoke-ViewerRpc {
    ...
    $output = (& $NodePath $ViewerPath $Action @RpcArguments 2>&1 | Out-String)
    if ($LASTEXITCODE -ne 0) { throw "Viewer RPC '$Action' failed: $($output.Trim())" }
...
try {
    ...
    [IO.File]::WriteAllText([string]$handoff.ackPath, $ackJson, (New-Object Text.UTF8Encoding($false)))
    ...
    Write-Host "Peer session '$($handoff.name)' is live. Type a line and press Enter to send; close this window to detach." -ForegroundColor Cyan
    Write-Host ''
    [long]$cursor = 0
    while ($true) {
    ...
    [IO.File]::WriteAllText(($HandoffPath + '.error'), $_.Exception.Message)
