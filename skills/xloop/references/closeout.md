...
3. Write accepted blockers and real proof failures to `raw/notes/YYYY-MM-DD-ll-<slug>.md` with `lesson_kind: lessons-learned` and frontmatter fields `supersedes:` and `superseded-by:` (blank by default). Derive each lesson from the commit subjects in the pinned range paired with the accepted findings in `REVIEW-LOG.md`: a subject beginning with a finding ID (`B1.3: ...`) names the blocker it closed and the fix that closed it, and a `No regression case for B1.3: <reason>` line in the report is itself a lesson. Do not re-derive lessons from prose alone. When this loop's lesson reverses an earlier lesson note (its fix contradicts the note's conclusion, or PLAN §D names it), set `supersedes: <earlier note stem>` here and set `superseded-by: <this note stem>` in the earlier note's frontmatter in the same step, so recon's grep stops serving the retired note. Promote the user's rulings into the same note (protocol §3.8): every `user_right` correction record in `.loop/QUESTIONS.md` and every question whose `Answer:` overrode `Recommended:` becomes one `[user-ruling]` line carrying the recommendation and the ruling side by side. `agent_right`, `unresolved`, and any record without an `Evidence:` line promote nothing. The list is clerical: run `scripts/loop-status.ps1 -Project <project> -Corrections` before this step and promote exactly what it prints.
